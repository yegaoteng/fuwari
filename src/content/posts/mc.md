---
title: 我的mcJava服务器
published: 2026-05-27
updated: 2026-05-27
description: 'mc教程'
image: 'https://app.webwd.de5.net/3.jpg'
tags: [mc]

draft: false 
lang: ''
---

# 我的世界
背景音:ferver pitch
<audio autoplay controls> 
<source src="http://f.ffurry.cc.cd/Fever%20Pitch.mp3" type="audio/mpeg"> 
</audio>


下载必要文件并安装:↓

[pcl启动器](https://gh-proxy.org/https://github.com/yegaoteng/myapp/releases/download/ppksp/Plain.Craft.Launcher.2.exe)

[1.19.2雾中人整合包](https://gh-proxy.org/https://github.com/yegaoteng/myapp/releases/download/ppksp/1.19.2gugugaga.zip)  |  [1.19.2雾中人整合包-备用](https://v4.gh-proxy.org/https://github.com/yegaoteng/myapp/releases/download/ppksp/1.19.2gugugaga.zip)



如果pcl启动器打不开下载[补丁](https://gh-proxy.org/https://github.com/yegaoteng/myapp/releases/download/ppksp/pcl.exe)

相关资源[ zl2启动器](https://gh-proxy.org/https://github.com/yegaoteng/myapp/releases/download/ppksp/Zalith.Launcher_2.4.8.apk)  |  [ zl2启动器备用](https://v4.gh-proxy.org/https://github.com/yegaoteng/myapp/releases/download/ppksp/Zalith.Launcher_2.4.8.apk)

渲染器:[mobileglues渲染器(高性能适合开光影大型资源包的时候用)](https://v4.gh-proxy.com/https://github.com/yegaoteng/myapp/releases/download/ppksp/MobileGlues_1.3.4.apk)


# 教程
### 电脑pcl2安装及整合包:
先下载java环境（上面有），在上方找到**pcl启动器**下载，**解压**，**打开**pcl启动器。

![pcl启动器](http://f.ffurry.cc.cd/1.png)

![](http://f.ffurry.cc.cd/2.png)

**下载**整合包（上面有），将下载的整合包直接**拖入**pcl启动器，安装好后返回启动器**首页**，在左侧**添加离线账号或正版账号(如果有)**。外置登录bing或百度有。

![pcl添加账号](http://f.ffurry.cc.cd/3.png)


## 注意:启动前先添加一个离线账户!

启动游戏！下面是服务器地址:

```
9666.fun:23098
```

littleskin外置登录API地址(也可使用离线登录):

```
https://littleskin.cn/api/yggdrasil
```

服务器地址(怕我忘记，你可以不用看):

[simpfun](https://simpfun.cn/)

`账号:jhhygt`

![](http://f.ffurry.cc.cd/bh.png)

**pcl2**同理，解决方法(选择一种):

* 1-关闭pcl2，单击鼠标左键选中pcl2，右键，选择以管理员身份运行。
* 2-清理缓存，重新在除系统盘外其他盘重新创建**全英目录**，如`D:\mc`，将pcl2及游戏数据文件夹，复制到里面重新用方法1启动。



#### 部分指令:

创造 `/gamemode creative`

生存 `/gamemode survival`

